...
impl Parser {
    ...
    pub fn is_match(&self, lexer: &mut dyn Lexer) -> bool {
        if self.elements.len() > 0 { self.elements.get(0).unwrap().is_match(lexer) } else { true }
    }
    ...
    pub fn or_owner(mut self, vec: Vec<&Rc<RefCell<Parser>>>) -> Self {
    ...
    pub fn insert_choice(mut self, parser: &Rc<RefCell<Parser>>) -> Self {
        ...
        self.or_owner(vec![parser, &otherwise])
    }
